...
export function seedAppData(db: DatabaseSync): void {
    ...
    db.prepare(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
        ...
        campaignId,
        'The Shattered Realm',
        'A dark fantasy campaign set in a world torn apart by ancient magic.',
        'Long ago, the Archmages of the Seven Towers attempted to reshape reality itself. Their hubris shattered the world into floating islands, each with its own twisted laws of nature. Now, centuries later, brave adventurers seek to reunite the fragments and restore balance.',
        null,
        gmUserId,
        'active',
        'invite-only',
    ...
    insertMember.run(
        ...
        'accepted',
        null,
        'Natural leader, tends to charge in first',
        now,
        now,
        now
    );
    ...
    insertMember.run(
        ...
        'accepted',
        null,
        'Curious about ancient magic, may be key to the plot',
        now,
        now,
        now
    );
    ...
    insertMember.run(
        ...
        'accepted',
        null,
        'Seeking redemption for a past failure',
        now,
        now,
        now
    );
    ...
    insertSession.run(
        ...
        'The party arrived at the Floating Citadel of Aethermoor and discovered the first fragment of the Sundering Stone. They fought through corrupted guardians and learned of the Seven Towers conspiracy.',
        new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString(), // 2 weeks ago
        240,
        JSON.stringify([char1Id, char2Id, char3Id]),
        gmUserId,
        now,
        now
    );
    ...
    insertSession.run(
        ...
        2,
        'Shadows in the Deep',
        'Descending into the Underdark beneath the citadel, the party encountered a cult worshipping the Void Between Worlds. They rescued a captured scholar who revealed the location of the second fragment.',
    ...
    insertSession.run(
        ...
        3,
        'The Crimson Wastes',
        'The party traveled to the Crimson Wastes, a desert island where time flows strangely. They battled sand wraiths and discovered an ancient temple containing the second fragment, but triggered a trap that alerted the enemy.',
        new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(), // 1 day ago
        180,
        JSON.stringify([char1Id, char2Id]),
        gmUserId,
        now,
        now
    );
    ...
    insertEvent.run(
        ...
        session3Id,
        'Sunblade Longsword',
        'A magical longsword found in the temple treasury',
        'loot',
        JSON.stringify({
            name: 'Sunblade',
            type: 'weapon',
    ...
    insertEvent.run(
        ...
        session3Id,
        'Session 3 XP Award',
        'Experience points for completing the Crimson Wastes quest',
        'xp_award',
        JSON.stringify({ amount: 1200 }),
        JSON.stringify([char1Id, char2Id]),
        null,
        null,
        gmUserId,
    ...
    insertEvent.run(
        ...
        session3Id,
        'The Enemy Awakens',
        'The trap triggered in the temple has alerted the Shadow Council to the party\'s progress. Dark agents are now hunting them.',
        'world_event',
        JSON.stringify({ severity: 'high', consequences: 'Increased enemy encounters' }),
        null,
        null,
        null,
        gmUserId,
    ...
    insertEvent.run(
        ...
        session3Id,
        'Desert Exhaustion',
        'The harsh conditions of the Crimson Wastes have taken their toll',
        'condition_applied',
        JSON.stringify({ condition: 'exhaustion', level: 1 }),
        JSON.stringify([char1Id, char2Id]),
        null,
        null,
        gmUserId,
    ...
    insertCompendium.run(
        compendium1Id,
        campaignId,
        'faction',
        'Cult of the Void',
        'A mysterious cult that worships the darkness between worlds',
        JSON.stringify({
        ...
        JSON.stringify(['enemy', 'cult', 'void']),
        1,
        gmUserId,
        now,
        now
    ...
    insertCompendium.run(
        ...
        campaignId,
        'item',
        'Fragment of the Sundering Stone',
        'A crystalline shard that pulses with reality-warping energy',
        JSON.stringify({
            type: 'quest_item',
            rarity: 'artifact',
            properties: {
        ...
        JSON.stringify(['artifact', 'quest', 'magical']),
        1,
        gmUserId,
        now,
        now
    ...
    insertCompendium.run(
        ...
        'faction',
        'The Shadow Council',
        'Secret organization manipulating events behind the scenes',
        JSON.stringify({
        ...
        JSON.stringify(['enemy', 'secret', 'bbeg']),
        0,
        gmUserId,
        now,
        now
    ...
    insertCompendium.run(
        ...
        'location',
        'Aethermoor Citadel',
        'A massive fortress floating in the sky, home to ancient guardians',
        JSON.stringify({
            region: 'The Skylands',
        ...
        JSON.stringify(['location', 'dungeon', 'skylands']),
        1,
        gmUserId,
        now,
        now
    ...
    insertCompendium.run(
        ...
        'spell',
        'Void Bolt',
        'A spell developed by the Cult of the Void',
        JSON.stringify({
            level: 2,
            school: 'Evocation',
            ...
            description:
                'You hurl a bolt of void energy at a creature within range. Make a ranged spell attack. On a hit, the target takes 3d10 necrotic damage and must succeed on a Wisdom saving throw or be frightened until the end of your next turn.',
        ...
        JSON.stringify(['spell', 'necrotic', 'void']),
        1,
        gmUserId,
        now,
        now
