...
import { renderAuthScreen as renderAuthScreenView } from './components/auth-screen.js';
import { renderSidebar as renderSidebarView } from './components/sidebar.js';
import { renderAdminPanel as renderAdminPanelView } from './components/admin-panel.js';
import { renderCreateFlowModal as renderCreateFlowModalView } from './components/create-flow-modal.js';
import { renderSheetEntryModalShell } from './components/sheet-entry-modal.js';
import { renderEditorShell } from './components/editor-shell.js';
import { renderCompendiumWindowShell } from './components/compendium-window.js';
import { renderDiceToolbarShell as renderDiceToolbarShellView } from './components/dice-toolbar.js';
...
function renderSheetEntryModal() {
  ...
  return renderSheetEntryModalShell({
    sectionLabel: modalSectionLabels[listName] || 'Entry',
    title,
    body,
  });
}
...
function renderDiceToolbarShell() {
  ...
  return renderDiceToolbarShellView({
    hidden: state.dice.hidden,
    open: state.dice.open,
    position,
    peekLabel: state.dice.sceneRoll?.formula || state.dice.formulaInput,
    railHtml: renderDiceToolbarRail(),
    controlsHtml: renderDiceToolbarControls(),
    summaryHtml: renderDiceSummary(),
    historyHtml: renderDiceHistory(),
  });
}
...
function renderEditor(character) {
  ...
  const editionOptionsHtml = EDITION_OPTIONS.map(
    ([value, label]) => `<option value="${value}" ${character?.data.edition === value ? 'selected' : ''}>${escapeHtml(label)}</option>`
  ).join('');
  const classOptionsHtml = (state.compendium?.classes || [])
    .map(
      (entry) => `
    ...
    .join('');
  const ancestryOptionsHtml = (state.compendium?.ancestries || [])
    .map(
      (entry) => `
    ...
    .join('');
  const backgroundOptionsHtml = (state.compendium?.backgrounds || [])
    .map(
      (entry) => `
  ...
  return renderEditorShell({
    ...
    charactersCount: state.characters.length,
    sheetContent,
    sheetTabsHtml: renderSheetTabs(),
    initiativeButtonHtml: character
      ? renderRollActionButton({
          label: 'Initiative',
          formula: describeRollFormula(1, 20, Number(character.data.initiative || 0)),
          text: 'Roll',
...
function renderCreateFlowModal() {
  return renderCreateFlowModalView({
    open: state.createFlowOpen,
  });
}
...
function renderCompendiumWindow() {
  ...
  return renderCompendiumWindowShell({
    open: state.compendiumOpen,
    hidden: state.compendiumHidden,
    pinned: state.compendiumPinned,
    dock: state.compendiumDock,
    type: state.compendiumType,
    query: state.compendiumQuery,
    searching: state.searching,
    detailIndex: state.compendiumDetailIndex,
    resultsHtml: state.compendiumResults.length === 0 ? '' : state.compendiumResults.map(renderCompendiumResult).join(''),
    detailHtml: renderCompendiumDetail(),
    activeCharacterLabel,
    floatingPosition,
    size: state.compendiumSize,
  });
}
    ...
    <div class="screen app-screen">
      <div class="app-shell">
        ${renderTopbarView({
          ...
          focusMode,
          activeCharacterName: focusMode ? character.data.name : '',
          charactersCount: state.characters.length,
